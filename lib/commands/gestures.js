...
/**
 * multiple click gesture
 * @this {HarmonyDriver}
 * @param {object} opts
 */
commands.mobileMultipleClickGesture = async function (opts) {
  let { elementId, x, y, times } = opts;
  if (elementId) {
    const point = await this.componentCache.get(elementId).getBoundsCenter();
    x = point.x;
    y = point.y;
  ...
  let cmdUnit = `-d ${x} ${y} -u ${x} ${y}`;
  let cmd = Array(times).fill(cmdUnit).join(' ');
  await this.driver.shell(`uinput -T ${cmd}`);
};
